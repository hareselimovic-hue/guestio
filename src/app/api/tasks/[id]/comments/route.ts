...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (process.env.RESEND_API_KEY) {
    const commenterName = session.user.name ?? session.user.email;
    const taskTyped = task as typeof task & { author: { id: string; email: string; name: string } };

    // Notify @mentioned users
    if (mentionIds && mentionIds.length > 0) {
      sendCommentMentionEmails(comment, taskTyped, commenterName, mentionIds).catch(() => {});
    }

    // Notify task author if they are not the commenter and not already @mentioned
    const authorAlreadyMentioned = (mentionIds ?? []).includes(taskTyped.author.id);
    if (taskTyped.author.id !== session.user.id && !authorAlreadyMentioned) {
      sendNewCommentEmail(comment, taskTyped, commenterName).catch(() => {});
    }
  }
...
async function sendNewCommentEmail(
  ...
  task: { id: string; content: string; property: { name: string } | null; author: { email: string; name: string } },
  ...
  await resend.emails.send({
    ...
    subject: `${commenterName} je komentirao/la tvoju poruku`,
    html: `<!DOCTYPE html>
<html>
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
        ...
        <tr><td style="background:#fff;border-radius:16px;border:1px solid #EDEDE9;padding:36px;">
          ...
          <p style="margin:0 0 20px;font-size:22px;font-weight:700;color:#262626;line-height:1.3;">
            ${commenterName} je komentirao/la tvoju poruku
          </p>
          ${propertyLine}
          <p style="margin:0 0 8px;font-size:12px;color:#9B9B9B;text-transform:uppercase;letter-spacing:0.5px;">Tvoja poruka</p>
